...
async function handleRegister() {
	...
	try {
		...
		const newUser: any = await $fetch('/api/user', {
			method: 'POST',
			body: {
				name: name.value,
				email: email.value,
		...
		tokenCookie.value = newUser.accessToken

		if(selectedPlan.value === 'pro') {
		  navigateTo(`/billing/${newUser.id}`, { replace: true })
		} else {
		  navigateTo(`/auth/api`, { replace: true })
		}
	}
	catch (error: any) {
		notify({
					...
					<div class="flex flex-col gap-6">
						...
						<div class="flex flex-col gap-3 md:flex-row">
							...
							<div class="flex flex-col gap-2">
								...
								<Input id="cpassword" v-model="confirmPassword" type="password"
